...
class CurrentStock:
    ...
    @classmethod
    def smpl_list_for_place_order(cls, customer):
        user_data = []
        cs_lst = []
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute("select * from current_stock where (status = 'RM' or status = 'HC' or status= 'WIP') and (customer=%s)  order by smpl_no asc",(customer,))
            user_data = cursor.fetchall()
    ...
    def customer_list_for_dispatch(cls):
        ...
        user_data = []
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute("select distinct customer from current_stock")
            user_data = cursor.fetchall()
        for lst in user_data:
