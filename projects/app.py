...
def main_menu(message=""):
    ...
    # In case the user is not logged in => no username is detected, the user cannot reach the main menu page and is redirected to the login page
...
# This is for incoming of smpl coil. The details are got from the xml file generated from Tally. The filename is to be entered here
@app.route('/incoming_input_smpl')
def incoming_input_smpl():
    return render_template('/incoming_enter_xml.html')


# This is for incoming of TR coil.
@app.route('/incoming_input_tr')
...
@app.route('/smpl_incoming')
def smpl_incoming():

    if request.method == 'POST':
        xml_filename = request.form['xml_filename']
    else:
        xml_filename = request.args.get('xml_filename')

    incoming_lst = Incoming.fromfile(xml_filename)
    # The details got from the XML file are updated to the database. The details are displayed for review and if any remarks are to be entered
...
def tr_incoming_commit():
    ...
    if request.method == 'POST':
        ...
        _incoming = Incoming(smpl_no, customer, incoming_date, thickness, width, length, grade, weight, numbers, mill, mill_id, incoming_remarks, unit)
        _incoming.savetodb()

    return render_template('/main_menu.html', message="Incoming details for " + smpl_no + " entered.")


# This function is to place raw material on hold. A list is obtained from current_stock table and displayed. The user has to pick from this list
...
# Once the RM to be placed on hold is selected. The status in current_stock is updated to RM - On Hold. With remarks updated in Incoming
...
# On selection and submit, the unit is changed in current_stock and remarks are updated in incoming about the transfer and when it was transferred
...
def upload_docs_submit():
    ...
    if len(return_file_list) > 1:
        return render_template('/upload_pick_smpl.html', message="These files were not uploaded. Please check the file extension and retry" + return_file_list)
    else:
...
def orders_by_machine():
    ...
    if customer_lst:
        return render_template('/processing_pick_customer.html', customer_lst = customer_lst, operation = operation)


@app.route('/RM_list_by_customer', methods=['GET', 'POST'])
def dispatch_list():
    if request.method == 'POST':
    ...
    cs_lst = CurrentStock.smpl_list_for_place_order(customer)
...
def processing_load():
    ...
    processing_detail_lst = ProcessingDetail.load_from_db(smpl_no, operation)
    processing_detail_display = []

    #return smpl_no + " " + operation
    # This is to display the previous processing for the sizes
...
'''
    ...
    if operation.startswith('CTL') or operation == 'Narrow CTL':
        if operation == 'CTL':
            _operation = 'CTL'
            machine = 'CTL 2'
        if operation == 'Narrow CTL':
            _operation = 'Narrow CTL'
            machine = 'CTL 3'

        return render_template('processing_ctl.html', incoming=incoming, operation=operation,
                               processing_details_lst=processing_detail_lst)

    '''if operation == 'Slitting' or operation == 'Mini Slitting':
        scrap = ms_width - _scrap
        if operation == 'Slitting':
            _operation = 'Slitting'
        if operation == 'Mini Slitting':
            _operation = 'Mini Slitting'
        return render_template('processing_slit.html', incoming=incoming, order=order,
                               order_detail_lst=order_detail_lst_by_operation, numbers=numbers, operation=_operation,
                               order_id=order_id, stage_no=stage_no, scrap=scrap, ms_width=ms_width,
                               completed_processing_wt=completed_processing_wt,
                               completed_processing_numbers=completed_processing_numbers)

    if operation == 'Reshearing':
        return render_template('processing_reshearing.html', incoming= incoming, order= order,
                               order_detail_lst=order_detail_lst_by_operation, numbers=numbers, operation=operation,
                               order_id=order_id, stage_no=stage_no, ms_width=ms_width,
                               completed_processing_wt=completed_processing_wt,
                               completed_processing_numbers=completed_processing_numbers)
...
# 3. Check if stage is complete. If yes, change status of next stage in order detail to ready and current stage to completed
...
def submit_processing():
    if request.method == 'POST':
        ...
        input_size_lst = request.form.getlist('input_material')
        cut_size_lst = request.form.getlist('cut_size')
        ...
        # For the smpl_no, order_no and stage_no get all the order details associated to it.
        order_detail_id_lst = OrderDetail.load_ids_for_same_stage_no(smpl_no,order_id,stage_no)

        # Processing object created and saved to db
        processing = Processing(order_id, smpl_no, operation, processing_date, start_time, end_time, setting_start_time, setting_end_time,
                                processing_time, setting_time, no_of_qc, no_of_helpers, names_of_qc, names_of_helpers,name_of_packer,stage_no)
        ...
        if operation == "CTL" or operation == "Reshearing" or operation == "Narrow CTL":
            for input_size, cut_size, actual_no_of_pieces, actual_no_of_packets, processed_wt, remarks, fg_yes_no, order_detail_id in zip(input_size_lst, cut_size_lst,actual_no_of_pieces_lst,actual_no_of_packets_lst,processed_wt_lst,remarks_lst, fg_yes_no_lst, order_detail_id_lst):
                ip_size = input_size.split('x')
                ms_width = ip_size[0]
                ms_length = ip_size[1]
                cc_size = cut_size.split('x')
                cut_width = cc_size[0]
                cut_length = cc_size[1]

                if processed_wt != '':
                    # Get mother size and cut size from the screen. Create processing detail and then update to db
                    processing_detail = ProcessingDetail(smpl_no,operation,machine,processing_id, order_detail_id, cut_width,
                                                     cut_length, actual_no_of_pieces, actual_no_of_packets, processed_wt,
                                                     remarks)
                    ...
                    cc_insert = CurrentStock.change_wt(smpl_no,cut_width,cut_length,processed_wt, actual_no_of_pieces, "plus")

                    # Unit of the material is decided based on the machine used to process the material.
                    # WARNING: This is bad programming
                    if machine == "CTL 2" or machine == "Slitting" or machine == "Mini Slitting" or machine == "Reshearing 5" or machine == "Reshearing 6" or machine == "Reshearing 7" or machine == "NCTL 1" or machine == "NCTL 2":
                        unit = '2'
                    else:
                        unit = '1'
                    ...
                    if cc_insert == "insert":
                        cs_cc = CurrentStock(smpl_no, customer, processed_wt, actual_no_of_pieces, thickness, cut_width,
                                             cut_length, fg_yes_no, grade, unit)
                        cs_cc.save_to_db()
        ...
        if operation == "Slitting" or operation == "Mini Slitting":
            input_size = input_size_lst[0]
            ip_size = input_size.split('x')
            ms_width = ip_size[0]
            ms_length = ip_size[1]

            # actual_no_of_pieces = length processed
            # actual_no_of_packets = no of parts
            # numbers = no of slits
            actual_no_of_pieces = float(actual_no_of_pieces_lst[0])
            actual_no_of_packets = int(actual_no_of_packets_lst[0])
            # processed_wt = processed_wt_lst[0]
            remarks = remarks_lst[0]
            for cut_size, fg_yes_no, order_detail_id, numbers in zip(
                    cut_size_lst, fg_yes_no_lst, order_detail_id_lst, numbers_lst):
                # Get cut size
                cc_size = cut_size.split('x')
                cut_width = float(cc_size[0])
                cut_length = float(cc_size[1])

                # Get number of coils produced for size = no of slits x no of parts
                no_of_coils = int(numbers) * actual_no_of_packets
                # This is processed weight for that size
                processed_wt = Decimal(thickness * cut_width * actual_no_of_pieces * 0.00000785 * float(no_of_coils))
                processed_wt = round(processed_wt,3)

                # Processing details updated to db
                if processed_wt > 0:
                    processing_detail = ProcessingDetail(smpl_no, operation, machine, processing_id, order_detail_id,
                                                         cut_width, cut_length, actual_no_of_pieces, no_of_coils,
                                                         processed_wt, remarks)
                    processing_detail.save_to_db()
                    ...
                    # Increase weight of cut material by processed weight. If cut material, doesn't already exist, the
                    # function returns insert => a new record has to be inserted
                    cc_insert = CurrentStock.change_wt(smpl_no, cut_width, cut_length, processed_wt, actual_no_of_pieces,
                                                       "plus")
                    ...
                    if cc_insert == "insert":
                        cs_cc = CurrentStock(smpl_no, customer, processed_wt, no_of_coils, thickness,
                                             cut_width, cut_length, fg_yes_no, grade, unit)
                        cs_cc.save_to_db()
            ...
            for slitter_batch, slitter_number in zip(slitter_batch_lst, slitter_number_lst):
                slitter_usage = SlitterUsage(processing_id, smpl_no, slitter_batch, slitter_number, actual_no_of_pieces, thickness)
...
def dispatch():
    ...
    for smpl, dispatch_nos, dispatch_qty, defective, no_of_packets in zip(dispatch_lst, dispatch_nos_lst, dispatch_quantity_lst, defectives_lst, dispatch_pkts_lst):
        smpl_details = smpl.split(',')
        smpl_no = smpl_details[1]
        cs_id = smpl_details[0]
...
def slitter_griding_entry():
    ...
    if submit_type == 'View Slitter Details':
        ...
        return render_template('slitter_batch_view.html', slitter_batch=slitter_batch, slitter_grinding_lst=slitter_grinding_lst)
...
def history_show_details():
    ...
    if smpl_no_lst:
        ...
        '''for smpl_no in smpl_no_lst:
            ...
            for order_id, order in order_lst:
                order_hdr_lst.append(order)
                order_detail_lst.append(OrderDetail.load_from_db(smpl_no, order_id))
