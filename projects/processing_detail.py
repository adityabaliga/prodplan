...
class ProcessingDetail:
    def __init__(self, smpl_no, operation, machine, processing_id, order_detail_id, cut_width, cut_length,
    ...
    def save_to_db(self):
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute("insert into processing_detail (smpl_no, operation, machine, processing_id, order_detail_id, "
                           "cut_width, cut_length, processed_numbers, processed_packets, processed_wt, remarks) values "
                           "(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (self.smpl_no, self.operation, self.machine,
                                                                            self.processing_id, self.order_detail_id,
                                                                            self.cut_width, self.cut_length,
                                                                            self.processed_numbers,
                                                                            self.processed_packets, self.processed_wt,
                                                                            self.remarks))
    ...
    def load_from_db(cls,smpl_no, operation):
        with CursorFromConnectionFromPool() as cursor:
            ...
            for lst in user_data:
                processing_dtl = ProcessingDetail(smpl_no=lst[1],operation=lst[2],machine=lst[3],processing_id=int(lst[4]),
                                                  order_detail_id=int(lst[5]), cut_width=Decimal(lst[6]),cut_length=Decimal(lst[7])
